...
public class 不同的二叉搜索树 {
    public static void main(String[] args) {
        ...
        /**
         * PS：首先这里需要明确一下什么叫二叉搜索树
         * 二叉搜索树：
         * 它或者是一颗空树，或者是具有下列性质的二叉树
         * 若它的左子树不空，则左子树的值均小于它的根节点上的值，
         * 若它的右子树不空，则右子树上的值均大于它的根节点上的值，
         * 它的左右子树也分别为二叉搜索树
         *
         * 
         * 然后题目是：
    ...
    public static int numTrees(int n) {
        ...
        for(int i=2;i<=n;i++){
            for(int j=1;j<=i;j++){
                //对于第i个节点，需要考虑1作为根节点，直到i为根节点的情况，所以需要累加
                //一共i个节点，对于根节点j时，左子树的节点个数为j-1，右子树的节点个树为i-j
                dp[i] += dp[j-1] * dp[i-j];
            }
        }
        return dp[n];
